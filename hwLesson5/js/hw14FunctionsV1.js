...
const num = prompt("Введите число которое нужно возвести в степень");
const stepen = prompt("Введите степень в которую возвести число")

alert(stepan(num, stepen));
...
function stepan(num, stepen = 1) {
    ...
    return `${stepen} не могу распознать число и понять какая степерь интересует`
}

function isCorrectNumber(value) {
    return !isNaN(Number(value)) && value !== null && value !== undefined && value.length > 0;
